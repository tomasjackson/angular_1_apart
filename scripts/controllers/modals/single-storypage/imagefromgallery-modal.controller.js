...
angular.module('frontendApp')
  .controller('ImportFromGallerySingleController', function ($uibModal, $scope , $sce , $state,
    MediaService, modalInstance, MediaStoryService, param, StoryManager, mainService) {
    ...
    *   Click Next button
    ...
    self.SaveButton = function () {
      ...
      if (self.selectedIdList.length === self.selectedCount) {
        ...
        var success_count = 0;
        for (var i = 0; i < self.selectedIdList.length; i++) {
          StoryManager.LinkStoryWithMedia(param.story.id , self.selectedIdList[i] , function(response) {
            if (response) {
              success_count ++;
            }
          });
        ...
        var scrollInterval = setInterval(function(){
            if ( success_count == self.selectedIdList.length ) {
